...
void Particle::addDrag(float const& drag)
{
	m_force.push_back(-drag*m_speed) ;
}
...
Vector2f Particle::getPreviousPosition() const
{
...
void Particle::updatePosition(float const& dt)
{
	...
	*/
	...
	m_speed = (m_position-m_previousPosition)/dt ; //now we need to update the speed by differentiating the position. (now we have the new position and the new speed : the particle moves !)
}
